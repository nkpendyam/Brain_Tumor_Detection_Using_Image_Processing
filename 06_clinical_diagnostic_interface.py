...
if _env_bool("BTD_SKIP_MODEL_LOAD", False):
    print(f"[{PROJECT_NAME}] Model loading skipped by BTD_SKIP_MODEL_LOAD=1.")
else:
...
# ── CSS (Apple-inspired clinical workspace) ───────────────────────────────────
...
/* ── Status ── */
.stat-row { display: grid; gap: 8px; grid-template-columns: repeat(3, 1fr); margin-bottom: 12px; }
.stat {
...
.chip-ok  {
  background: var(--green-bg); border-color: rgba(36, 138, 61, .22);
  color: var(--green);
}
.chip-off {
  background: var(--red-bg); border-color: rgba(215, 0, 21, .18);
  color: var(--red);
}
...
.warn-box {
  ...
  background: var(--red-bg); border: 1px solid rgba(215, 0, 21, .18);
  color: var(--red); font-size: .82rem; line-height: 1.45;
}
...
.report-out .prose p,
.report-out .prose li,
.report-out .prose blockquote,
.report-out .prose div,
.report-out .prose span {
...
.report-out .prose strong {
  color: var(--text) !important;
  font-weight: 700 !important;
  opacity: 1 !important;
}
.report-out .prose em { color: var(--text-3) !important; }
...
@media (max-width: 768px) {
  ...
  .app-metrics { grid-template-columns: repeat(3, 1fr); min-width: 0; }
  .metric { padding: 8px; }
...
.pipe-step {
  ...
  font-size: .85rem; color: var(--text-2); padding: 8px 0;
...
with gr.Blocks(title="Brain Tumor Detection") as app:
    ...
    <div id="app-header">
      <div class="app-title">
        ...
        <p>Clinical AI workspace for MRI, CT, DICOM, and NIfTI studies.</p>
      </div>
      <div class="app-metrics">
        <div class="metric"><span class="v">98.25%</span><span class="l">Accuracy</span></div>
        <div class="metric"><span class="v">0.9783</span><span class="l">Macro F1</span></div>
        <div class="metric"><span class="v">300</span><span class="l">Max slices</span></div>
      </div>
    </div>
    """)
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=4, min_width=320):
            with gr.Group(elem_classes=["panel"]):
                gr.HTML('<div class="ptitle"><span>1</span>Upload study</div>')
                upload = gr.File(
                    file_count="multiple",
            ...
            with gr.Group(elem_classes=["panel"]):
                gr.HTML('<div class="ptitle"><span>2</span>Model status</div>')
                status_out = gr.HTML(_status_html())

            with gr.Group(elem_classes=["panel"]):
                gr.HTML("""
                <div class="ptitle"><span>3</span>Pipeline</div>
